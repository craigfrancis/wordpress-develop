...
class Tests_Query_DateQuery extends WP_UnitTestCase {
	...
	public function test_date_params_monthnum_m_duplicate() {
		...
		$this->assertStringContainsString( 'MONTH( ' . $wpdb->escape_identifier( $wpdb->posts ) . '.`post_date` ) = 5', $this->q->request );
		$this->assertStringNotContainsString( 'MONTH( ' . $wpdb->escape_identifier( $wpdb->posts ) . '.`post_date` ) = 9', $this->q->request );
	}
	...
	public function test_date_params_week_w_duplicate() {
		...
		$this->assertStringContainsString( 'WEEK( ' . $wpdb->escape_identifier( $wpdb->posts ) . '.`post_date`, 1 ) = 43', $this->q->request );
		$this->assertStringNotContainsString( 'WEEK( ' . $wpdb->escape_identifier( $wpdb->posts ) . '.`post_date`, 1 ) = 42', $this->q->request );
	}
