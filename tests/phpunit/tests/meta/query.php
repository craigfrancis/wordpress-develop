...
class Tests_Meta_Query extends WP_UnitTestCase {
	...
	public function test_not_exists() {
		...
		$sql = $query->get_sql( 'post', $wpdb->posts, 'ID', $this );
		$this->assertStringNotContainsString( $wpdb->escape_identifier( $wpdb->postmeta ) . ".meta_key = 'exclude'\nOR", $sql['where'] );
		$this->assertStringContainsString( $wpdb->escape_identifier( $wpdb->postmeta ) . '.`post_id` IS NULL', $sql['where'] );
	}
	...
	public function test_empty_compare() {
		...
		$this->assertMatchesRegularExpression( "/" . $wpdb->escape_identifier( $wpdb->postmeta ) . "\.meta_key = \'exclude\'\s+OR/", $sql['where'] );
		$this->assertStringNotContainsString( $wpdb->escape_identifier( $wpdb->postmeta ) . '.post_id IS NULL', $sql['where'] );
	}
